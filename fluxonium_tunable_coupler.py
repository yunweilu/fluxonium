...
class FluxoniumTunableCouplerFloating(base.QubitBaseClass, serializers.Serializable):
    ...
    def generate_coupled_system_sweetspot(self):
        ...
        fluxonium_a.flux = 0.5
        fluxonium_b.flux = 0.5
        hilbert_space.add_interaction(g=-self.ELa * offset_flux_a, op1=phi_a, id_str="Phi_a*phi_a")
        hilbert_space.add_interaction(g=0.5 * self.ELa * offset_flux_a, op1=phi_plus, id_str="Phi_a*phi_plus")
        hilbert_space.add_interaction(g=0.5 * self.ELa * offset_flux_a, op1=phi_minus, id_str="Phi_a*phi_minus")
        hilbert_space.add_interaction(g=-self.ELb * offset_flux_b, op1=phi_b, id_str="Phi_b*phi_b")
        hilbert_space.add_interaction(g=0.5 * self.ELb * offset_flux_b, op1=phi_plus, id_str="Phi_b*phi_plus")
        hilbert_space.add_interaction(g=-0.5 * self.ELb * offset_flux_b, op1=phi_minus, id_str="Phi_b*phi_minus")
        return hilbert_space
    ...
    def hilbert_space_at_sweetspot(self, flux_shift_a=None, flux_shift_b=None) -> HilbertSpace:
    ...
    def basis_change(op, evecs, hilbert_space, subsystem):
        ...
        if isinstance(evecs_bare, NamedSlotsNdarray):
            evecs_bare = evecs_bare[0]
    ...
    @staticmethod
    def basis_change_Danyang(op, evecs, hilbert_space, subsystem):
        op_id_wrap = identity_wrap(op, subsystem, hilbert_space.subsystem_list)
        if evecs is None:
            return op_id_wrap
        else:
            evecs_array = np.array([ev.full()[:, 0] for ev in evecs])
            op_new_basis = Qobj(evecs_array @ op_id_wrap.full() @ evecs_array.T)
            return op_new_basis

    def operators_at_sweetspot(
        self,
        num_states: int | None = 4,     # if None, do not truncate
        flux_shift_a=None,
        flux_shift_b=None,
        standardize_sign=True,
        remove_unnecessary_states=False,
    ):
        # hilbert_space = self.hilbert_space_at_sweetspot(flux_shift_a, flux_shift_b)
        if flux_shift_a is not None:
            self.flux_a = 0.5 + flux_shift_a
        if flux_shift_b is not None:
            self.flux_b = 0.5 + flux_shift_b

        hilbert_space = self.generate_coupled_system_sweetspot()
        ...
        if num_states is not None:
            # perform basis transformation & truncation
            hilbert_space.generate_lookup()

            evals = Qobj(hilbert_space["evals"][0][0:num_states])
            evecs = hilbert_space["evecs"][0][0:num_states]

            # dressed states are usually defined with bare states, so their phase is important
            # for example, dressed state |0, 1> should be defined as p|0, 1> + ..., where p 
            # is real and greater than a threshold.
            if standardize_sign:
                evecs_w_sign = []
                for drs_idx, evec in enumerate(evecs):
                    bare_idx_ravel = np.ravel_multi_index(hilbert_space.bare_index(drs_idx), hilbert_space.subsystem_dims)
                    evecs_phase = evec[bare_idx_ravel, 0] / np.abs(evec[bare_idx_ravel, 0])
                    evec = evec / evecs_phase
                    evecs_w_sign.append(evec)
            else:
                evecs_w_sign = evecs

            evals = evals.full()
            evals_zeroed = evals[:, 0] - evals[0, 0]
            H_0 = Qobj(2.0 * np.pi * np.diag(evals_zeroed[0:num_states]))

        else: 
            evecs_w_sign = None      # tells basis_change_Danyang to not do basis change
            H_0 = 2.0 * np.pi * hilbert_space.hamiltonian()

        phi_a = self.basis_change_Danyang(
            phi_a, evecs_w_sign, hilbert_space, fluxonium_a
        )
        phi_b = self.basis_change_Danyang(
            phi_b, evecs_w_sign, hilbert_space, fluxonium_b
        )
        phi_minus = self.basis_change_Danyang(
            phi_minus, evecs_w_sign, hilbert_space, fluxonium_minus
        )
        phi_plus = self.basis_change_Danyang(
            phi_plus, evecs_w_sign, hilbert_space, h_o_plus
        )
        ...
        if remove_unnecessary_states and num_states is not None:
            ...
            for bare_label in bad_bare_labels:
                ...
                if isinstance(bad_dressed_index, NamedSlotsNdarray):
                    bad_dressed_index = bad_dressed_index[0]
    ...
    def find_sweetspot_by_hybr(
        ...
        def hybridization(param):   
            ...
            flux_c = param["flux_c"]
            self.flux_c = flux_c
            H_0_full, H_a_full, H_b_full, H_c_full = self.operators_at_sweetspot(
                num_states=4,
                flux_shift_a=flux_s,
                flux_shift_b=-flux_s,
            ...
            return np.sqrt((
                np.abs(H_a_full[0, 2] / H_a_full[0, 1])**2 
                + np.abs(H_a_full[1, 3] / H_a_full[0, 1])**2
                + np.abs(H_b_full[0, 1] / H_b_full[0, 2])**2
                + np.abs(H_b_full[2, 3] / H_b_full[0, 2])**2
            ) / 4)
    ...
    def fluxonium_a(self) -> Fluxonium:
        return Fluxonium(
            self.EJa,
    ...
    def fluxonium_b(self) -> Fluxonium:
        return Fluxonium(
            self.EJb,
    ...
    def fluxonium_minus(self) -> Fluxonium:
        return Fluxonium(
            self.EJC,
...
class ConstructFullPulse(serializers.Serializable):
    ...
    def fix_w_single_q_gates(gate_, which_Z_exclude=2, const_angle_val=0.0, which_gate='sqrtiswap'):
        ...
        elif which_gate == 'iswap':
            angles_to_correct = (
                np.array([-alpha, 0, gamma - np.pi / 2])
                - const_angle_val * const_angle_col
            )
        elif which_gate == 'bswap':
            beta = cmath.phase(gate_[1, 1])
            delta = cmath.phase(gate_[0, 3])
            angles_to_correct = (
                np.array([-beta, 0, delta - np.pi/2])
            )
        else:
    ...
    @staticmethod
    def amp_from_freq_iswap(omega, omega_d, n=1):
        return np.abs(
            0.25
            * np.pi
            * (omega_d ** 2 - omega ** 2)
    ...
    @staticmethod
    def amp_from_freq_bswap(omega, omega_d, n=1):
        return np.abs(
            0.25
            * np.pi
            * (omega_d ** 2 - omega ** 2)
    ...
    def drive_freq_iswap(self, n=1, m=4):
        omega_a = np.real(self.H_0[2, 2])
        omega_b = np.real(self.H_0[1, 1])
        return n * (omega_a + omega_b) / m
    
    def drive_freq_bswap(self, n=1, m=4):
        omega_a = np.real(self.H_0[2, 2])
        omega_b = np.real(self.H_0[1, 1])
        return n * (omega_a - omega_b) / m
